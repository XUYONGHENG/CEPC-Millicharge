...
def cut_cepc_advance(p,mass=100):
    if p[8] < np.min([240-2*mass,80]) and m_missing_240_xx(p) > np.max([2*mass,120]):
        if cut_12_cepc_1_irredu(p):
            if cut_custom(p):
                return True
    return False
...
def cut_cepc_advance_disposed(p,mass=1,mode='h'):
