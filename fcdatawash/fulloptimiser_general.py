...
def advcut_applier(signal,mode='h'):
    ...
    print(np.sum(bge[:, -1])+ 2 * np.sum(bgm[:, -1]), np.sum(sig[:, -1]))
...
def rawsb(s,b,l=5e6):
    return s*l/np.sqrt(b*l)


def fulloptimiser(ratio,shist,bghist,luminosity = 5e6, ff=0):
    ...
    if a != 0 and b == 0:
        ...
        print(flatorderd[:a+1].shape)
        flatorderd = np.concatenate((flatorderd[:a+1],flatorderd[b+1:-1]))
        print(flatorderd.shape)
        flatorderd = flatorderd[::-1]
        print('no zero')
        # print(flatorderd)
    ...
    for i in range(flatorderd.shape[0]):
        ...
        if ff == 0:
            ...
            ep = rawsb(s = stotal, b=bgtotal, l=luminosity)
        epsilons[i] = ep
        # print(type(ep))
    # print(epsilons)
    # epsilons = epsilons[1:]
    if ff:
        epsilons = epsilons[1:]
        epsilons[epsilons == np.inf] = 200
    else:
        epsilons = epsilons[1:]
        epsilons[epsilons == np.inf] = 1
        epsilons[epsilons == -np.inf] = 1
