...
ax1.legend((l1d,l1,l1a,l1b,),(r'2.5 $ab^{-1}$',r'5 $ab^{-1}$',
                              r'10 $ab^{-1}$',r'20 $ab^{-1}$',),loc='lower right',fontsize=12)
...
a2 = np.load('../plotCSV/sbdotscepcz.npy')
b2 = np.load('../plotCSV/sb2dots1.npy')
c2 = np.load('../plotCSV/sbdotscepcw.npy')
d2 = np.vstack((a2,b2[-5:]))

l22, = ax2.plot(a2[:,0],a2[:,1],linewidth=3,linestyle='solid',c='red')
l21, = ax2.plot(mass,b2[:,1],linewidth=3,linestyle='solid',c='black')
l23, = ax2.plot(c2[:,0],c2[:,1],linewidth=3,linestyle=':',c='blue')

l24, = ax2.plot(d2[:,0],d2[:,1],linewidth=5,linestyle='dashed',c='green',alpha=0.5)


lc2, = ax2.plot(datac[:,0],datac[:,1],linewidth=0,linestyle='solid',color='gray')
lc22 = ax2.plot(datac2[:,0],datac2[:,1],linewidth=0,color = 'red')

ax2.fill_between(datac[:,0],datac[:,1],y2=10,color='blue',alpha=0.6)
ax2.fill_between(datac2[:,0],datac2[:,1],y2=10,color='red',alpha=0.6)
# ax.legend((l1,l2),('FormCalc Monte Carlo','FeynCalc Analytic'),fontsize=24)

# ax2.text(1,0.6,s='CEPC constraints on milliQ',fontsize=36)
# ax2.text(1,0.,s=r'$\sqrt{S}=91.2GeV$,CEPC detector cuts & Advanced cuts',fontsize=36)
ax2.set_xlim((0.5,120))
ax2.set_ylim((1e-2,1))

ax2.legend((l21,l22,l23,l24),('Higgs Fatory','Z-pole','W+W-','Combined Best'),loc='lower right',fontsize=12)
...
ax2.set_ylabel(r'$\epsilon$',fontdict={'fontsize':25})
