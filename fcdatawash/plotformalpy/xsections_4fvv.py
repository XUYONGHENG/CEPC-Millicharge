...
hvvx = np.array(pd.read_table('../plotCSV/4fvvxsectiondots.csv',header=None))
zvvx = np.array(pd.read_table('../plotCSV/4fvvxsectiondotsz.csv',header=None))
wvvx = np.array(pd.read_table('../plotCSV/4fvvxsectiondotsw.csv',header=None))
print(wvvx[:,0])
...
ax.text(3,8,r'Z-pole,$\sqrt{s}$=91.2 GeV',fontsize=20,color='blue')
ax.text(3,27,r'$W^+W^-$,$\sqrt{s}$=160 GeV',fontsize=20,color='red')
ax.text(3,70,r'Higgs Factory,$\sqrt{s}$=240 GeV',fontsize=20,color='black')
# ax.text(2,12,r'Vector 4-fermion interaction',fontsize=20)
ax.text(2,5,r'$L$=$\frac{1}{\Lambda^2}$$\bar{\chi}\gamma_\mu\chi\bar{e}\gamma^\mu e$',fontsize=20)
