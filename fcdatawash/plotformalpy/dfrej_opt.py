...
a,b,c = np.column_stack((mass,a)),np.column_stack((mass[:b.shape[0]],b)),c[:,:2]
print(c.shape)

a = np.vstack((a[:-2],np.array([[74.62,5e-4],[74.87,4.98e-4],[75,8e-6],[75.12,11.8e-4]]),a[-2:]))
# print(a)
# input('halt')
#
#
...
ax.text(2,3e-4,s=r"Optimised CEPC constraints on $g_f^V$",fontsize=24)
ax.text(2,1.5e-4,s=r"$m_{Z'}=150$ GeV,$g_\chi^A=g_f^A=0,g_\chi^V=1$",fontsize=24)
...
ax.text(2,0.0018,r'Higgs Factory $\sqrt{s}=240$ GeV',Fontsize=24,color='black')
ax.text(2,0.008,r'W+W- $\sqrt{s}=160$ GeV',Fontsize=24,color='red')
ax.text(2,0.02,r'Z-pole $\sqrt{s}=91.2$ GeV',Fontsize=24,color='blue')
...
plt.savefig('../figformal/dfrej_opt_2.pdf',format='pdf')
plt.show()c