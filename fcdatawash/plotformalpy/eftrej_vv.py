...
az,bz,cz,dz = np.vstack((az[:-1],zadd)),np.vstack((bz[:-1],zadd)),np.vstack((cz,zadd)),np.vstack((dz[:-1],zadd))
...
print(d[0],d[10])
l1a, = ax.plot(d[:,0],d[:,1],linewidth=3,linestyle='solid',color='black')
l1b, = ax.plot(dw[:,0],dw[:,1],linewidth=3,linestyle='dashed',color='red')
l1d, = ax.plot(dz[:,0],dz[:,1],linewidth=3,linestyle=':',color='blue')
...
ax.text(2,850,r'$W^+W^-$ $\sqrt{s}$=160 GeV',fontsize=20,color='red')
ax.text(2,1080,r'$Z$-pole $\sqrt{s}$=91.2 GeV',fontsize=20,color='blue')
ax.text(5,550,r'Vector',fontsize=24)
...
plt.savefig('../figformal/eftrej_vv_2.pdf',format='pdf')
