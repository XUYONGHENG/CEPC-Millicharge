...
az,bz,cz,dz = np.vstack((az[:-1],zadd)),np.vstack((bz[:-1],zadd)),np.vstack((cz,zadd)),np.vstack((dz,zadd))
...
l1a, = ax.plot(b[:,0],b[:,1],linewidth=3,linestyle='solid',color='black')
l1b, = ax.plot(bw[:,0],bw[:,1],linewidth=3,linestyle='dashed',color='red')
l1d, = ax.plot(bz[:,0],bz[:,1],linewidth=3,linestyle=':',color='blue')
...
ax.text(2,1300,r'Higgs Factory $\sqrt{s}$=240 GeV',fontsize=20,color='black')
ax.text(2,800,r'$W^+W^-$ $\sqrt{s}$=160 GeV',fontsize=20,color='red')
ax.text(2,1120,r'$Z$-pole $\sqrt{s}$=91.2 GeV',fontsize=20,color='blue')
ax.text(5,400,'Scalar(s)',fontsize=24)
...
plt.savefig('../figformal/eftrej_ss_2.pdf',format='pdf')
