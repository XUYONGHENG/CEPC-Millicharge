...
def two_three_phase_space_dot_rambo(s_sqrt,masses):
    ...
    p1,p2,p3 = p_1.components,p_2.components,np.array([s_sqrt,0,0,0])-p_1.components-p_2.components
    # b = np.array([lt.cos_theta_z(p2),lt.cos_theta_z(p3),lt.cos_theta_z(p1),p1[0]])
    # if cuts.cut_besiii_4(b):
    #     print("Beep!\n")
    #     print(b)
    #     print(p1,p2,p3)
    #     print("Beep!\n")
    print(lt.cos_theta_z(p2),lt.cos_theta_z(p3),lt.cos_theta_z(p1),p1[0])
    return lt.cos_theta_z(p2),lt.cos_theta_z(p3),lt.cos_theta_z(p1),p1[0]

# a = np.zeros(shape=(1000000,4))
# for i in range(1000000):
#     a[i] = two_three_phase_space_dot_rambo(s_sqrt=4,masses=[0,0,0])
#     if i%10000 == 0:
#         print(i)
# np.savetxt('shit.csv',a,delimiter=', ')
# input('x')
...
def phase_space_23_dot_alpha(s_sqrt):
    while 1:
        raw_dot = np.random.rand(5, )
        ...
        #If energy cut on Fermions is needed, it could be added here.
        ...
        # if np.abs((e_1*cos_theta1+e_2*cos_theta2)/e_3)<0.984:
        #     continue
...
# start = time.clock()
# for i in range(1000):
#     phase_space_23_dot_alpha(s_sqrt=500)
# end = time.clock()
# print(end-start)
def phase_space_23_dot_beta(s_sqrt):
    while 1:
        raw_dot = np.random.rand(5, )
...
def phase_space_23_dot_gamma(s_sqrt):
    raw_dot = np.random.rand(5, )
    mv =  s_sqrt*raw_dot[0]
    # energy of final state photon. Must be > 10 GeV to avoid soft photon singularity and be seen by detectors.
    e_1 = (s_sqrt ** 2 - mv ** 2) / (2 * s_sqrt)
    ...
    phi_1 = 2 * np.pi * raw_dot[2]
    phi_12 = 2 * np.pi * raw_dot[3]
    theta1 = np.pi * raw_dot[4]
    #photon angle cut.
    sin_theta1 = np.sin(theta1)
    cos_theta1 = np.cos(theta1)
    cos_theta12 = ((s_sqrt - e_1 - e_2) ** 2 - e_1 ** 2 - e_2 ** 2) / (2 * e_1 * e_2)
    cos_theta2 = cos_theta1 * cos_theta12 + sin_theta1 * np.sin(np.arccos(cos_theta12)) * np.cos(phi_12)
    # if np.abs(cos_theta2)<0.984:
    #     continue
    sin_theta2 = np.sin(np.arccos(cos_theta2))
    phi_2 = phi_1 - np.arccos((cos_theta12 - cos_theta2 * cos_theta1) / (sin_theta1 * sin_theta2))
    ...
    #     continue
    k1 = e_1 * np.array([1, sin_theta1 * np.cos(phi_1), sin_theta1 * np.sin(phi_1), cos_theta1])
    k2 = e_2 * np.array([1, sin_theta2 * np.cos(phi_2), sin_theta2 * np.sin(phi_2), cos_theta2])
    k3 = np.array([s_sqrt, 0, 0, 0]) - k1 - k2
    cos_theta3 = lt.cos_theta_z(k3)
    # print(k1+k2+k3)
    # print(k1[0]**2-k1[1]**2-k1[2]**2-k1[3]**2)
    # print(k2[0]**2-k2[1]**2-k2[2]**2-k2[3]**2)
    # print(k3[0] ** 2 - k3[1] ** 2 - k3[2] ** 2 - k3[3] ** 2)
    # if np.random.randint(1,):
    return cos_theta3,cos_theta2,cos_theta1,e_1


# phase_space_23_dot_gamma(4)
# print(phase_space_23_dot_nt2_massles`1                                                s_cut(s_sqrt=500).get_vector())
# start = time.time()
# # a = np.zeros(4,)
# # b = np.zeros(4,)
# # d = np.zeros(4,)
# for i in range(1000000):
#     c = phase_space_23_dot_gamma(4)
# #     a += c[0]
# #     b += c[1]
# #     d += c[2]
# end = time.time()
