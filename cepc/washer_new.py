...
def lhegz_wash_to_ob_auto_general(datafile,cut,savename,energy,s=False,smearer=smear.no_smear):
    survivors = 0
    with open(datafile) as file_object:
        a = file_object.readlines()
    length = len(a)
...
def general_dat_parser(datafile):
    ...
    with open("/store/tmp/generaltmp.csv", 'w') as f:
        f.writelines(disposed)
    a = np.array(pd.read_csv("/store/tmp/generaltmp.csv"),header=None)
    os.system('rm /store/tmp/generaltmp.csv')
    print(a.shape)
    return a
...
if __name__ == '__main__':
    ...
    # lhegz_wash_to_ob_auto(datafile='/store/4fermion2/mg5runs/xxeeav2/unweighted_events.lhe',
    #                       savename='cepc4fav_h_2_m1',energy=240,cut=cut.cut_nocut,
    #                       s=True,smearer=smear.smear_cepc_0)
    # input('halt')
    for i in range(201):
        fcoutput_to_ob_w(datafile='/store/milliq/cepch/mqhnew132m%d.dat'%i,
                         weightfile='/store/milliq/cepch/mqhnew132m%dw.dat'%i,
                         cut=cut.cut_nocut,savename='mqhnew132_%d'%i)
    # for i in range(100):
    #     lhegz_wash_to_ob_auto(datafile='/store/milliq/mg5runs/mqrun2/%d_new.lhe'%i,
    #                                savename = 'mqnew_%d'%i,energy=240,
