...
if __name__ == '__main__':
    ...
    #     data = np.load('/store/disposed/finalpush/zp/h/a/mz150a_mx%d_50-100.lhed.npy'%(i+1))
    #
    #     print(np.sum(data[:,-1]))
    #
    ...
    # print(dots.shape)
    # print(dots)
    # np.save('../plotCSV/zp_a_h_formal_50-100.npy',dots)
    dots = np.zeros(shape=(9,2))
    masses = np.linspace(1,49,9,endpoint=False)
    for i in range(9):
        data = np.load('/store/disposed/finalpush/zp/h/a/mz150a_mx%d_1-50d.npy'%(i+1))
        ...
        print(dots.shape)
        print(dots)
        np.save('../plotCSV/zp_a_h_formal_1-50.npy',dots)
