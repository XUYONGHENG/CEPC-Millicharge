...
if __name__ == '__main__':
    ...
    dots = np.zeros(shape=(15, 2))

    # 0.1 - 1 add-ons. This is ugly, I know.

    masses = np.linspace(0,70,15,endpoint=True)
    print(masses)
    # input('halt')
    for i in range(15):
        ...
        data = np.load('/store/disposed/finalpush/zp/w/v/mzw150v_mx%d_1-70d.npy'%(i+1))

        print(np.sum(data[:,-1]))

        dots[i,1] = signal_significance(s=data,
                            bg=bgraw,mass=masses[i],
                            cut=cut.cut_zp_iv_150_3w,
                            dataamount=2.6e6,splusb=False,is_epsilon=True)[0]
        dots[i,0] = masses[i]
        print(dots[i,1],'exclusion\n')

    print(dots.shape)
    print(dots)
    np.save('../../plotCSV/zp_v_w_formal_1-70.npy',dots)
