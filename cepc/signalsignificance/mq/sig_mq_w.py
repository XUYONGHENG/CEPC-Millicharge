...
if __name__ == '__main__':
    ...
    dots = np.zeros(shape=(67, 2))
    dots0 = np.zeros(shape=(2, 2))

    # 0.1 - 1 add-ons. This is ugly, I know.

    dots0[0, 0], dots0[1, 0] = 0.1, 0.5
    dots0[0, 1] = 1e2 * signal_significance(s=np.load('/store/disposed/finalpush/milliq/w/mqw132m0.1d.npy'),
                                            bg=bgraw, mass=0.1, cut=cut.cut_cepcw_advance_disposed, dataamount=2.6e6,
                                            splusb=False)[0]
    dots0[1, 1] = 1e2 * signal_significance(s=np.load('/store/disposed/finalpush/milliq/w/mqw132m0.5d.npy'),
                                            bg=bgraw, mass=0.5, cut=cut.cut_cepcw_advance_disposed, dataamount=2.6e6,
                                            splusb=False)[0]
    ...
    for i in range(67):
        ...
        data = np.load('/store/disposed/finalpush/milliq/w/mqwx132%dd.npy' % (i+1))
        ...
        dots[i, 1] = signal_significance(s=data,
                                         ...
                                         cut=cut.cut_cepcw_advance_disposed,
                                         dataamount=2.6e6, splusb=False)[0]
        dots[i, 0] = masses[i]
    ...
    print(dots.shape)
    print(dots)
    np.save('../plotCSV/mqw_formal_0.1-67.npy', dots)
